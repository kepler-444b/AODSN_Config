#include "model_panel_2key.h"
#include "ui_model_panel_2key.h"

ModelPanel2Key::ModelPanel2Key(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ModelPanel2Key)
{
    ui->setupUi(this);
    m_buttons.clear();
...
ModelPanel2Key::~ModelPanel2Key()
{
    delete ui;
}

void ModelPanel2Key::SetKeyNames(const QStringList &names)
{
...
void ModelPanel2Key::SetPanelAddr(const uint8_t addr)
{
    ui->addr->setText(QString("地址:%1").arg(addr));
    m_addr = addr;
}

bool ModelPanel2Key::eventFilter(QObject *obj, QEvent *event)
{
    ...
    case 1: hwIndices << 2 << 3; break; // 键2 -> 2, 3
