...
static uint8_t getKeyBitMask(int keyIndex, int keyCount)
{
    ...
    else if (keyCount == 3) // 三键面板特殊映射
    {
        switch (keyIndex)
        {
        case 0: return (1u << 0) | (1u << 1); // 0键 -> 0,1
        case 1: return (1u << 4) | (1u << 5); // 1键 -> 4,5
        case 2: return (1u << 2) | (1u << 3); // 2键 -> 2,3
        default: return 0;
        }
    }
    else if (keyCount == 2)
...
// 根据设备状态组成场景数据
void AppProtocolSetConfig::recv_scene_data(const QVector<panel_data_t>& panels, const QVector<led_data_t>& leds, const QVector<relay_data_t>& relays, uint8_t scene_id , QString name , bool &success)
{
    bool exists = std::any_of(m_scene_save.begin(), m_scene_save.end(), [scene_id](const scene_save_t& s){ return s.scene_id == scene_id;});
    if (exists)
    {
        // 弹出询问窗口，让用户选择是否覆盖替换
        ...
        {
            // 用户选择替换：直接调用功能完善后的 deleteScene 把它全清掉
...
{
    bool exists = std::any_of(m_bind_save.begin(), m_bind_save.end(), [panelAddr, keyIndex, isOpen](const bind_save_t &b){
        if (b.bind_data.size() >= 4) {
            uint8_t panel_addr = static_cast<uint8_t>(b.bind_data[0]);
            uint8_t key_num    = static_cast<uint8_t>(b.bind_data[1]);
            uint8_t key_state  = static_cast<uint8_t>(b.bind_data[2]);
            return panel_addr == panelAddr && key_num == keyIndex && key_state == isOpen;
        }
        return false;
    });
...
void AppProtocolSetConfig::bulidSceneData(const QVector<panel_data_t>& panels, const QVector<led_data_t>& leds, const QVector<relay_data_t>& relays, uint8_t scene_id, QString scene_name)
{
    ...
    bytes.append(static_cast<char>(0xFF));
    qDebug() << "panel ctrl   :" << QByteArray(reinterpret_cast<char*>(ctrl.data()), 32).toHex(' ').toUpper();
    qDebug() << "panel status :" << QByteArray(reinterpret_cast<char*>(status.data()), 32).toHex(' ').toUpper();
    qDebug() << "panel reserve :" << QByteArray(reinterpret_cast<char*>(status.data()), 32).toHex(' ').toUpper();

    scene_save_t scene;
    scene.scene_name = scene_name;
    scene.scene_data = bytes;  // 如果 scene_data 是 QByteArray 类型
    scene.scene_id   = scene_id;
    m_scene_save.append(scene);
...
void AppProtocolSetConfig::printConfigData() const
{
    if (m_scene_save.isEmpty()) {
        qDebug() << "m_sceneData 为空";
        return;
    }
    qDebug() << "===== 打印 m_sceneData，共" << m_scene_save.size() << "条场景 =====";
    for (int sceneIndex = 0; sceneIndex < m_scene_save.size(); ++sceneIndex) {
        const scene_save_t &scene = m_scene_save[sceneIndex];

        // 打印场景名称
        qDebug() << QString("场景 %1: 名称 %2").arg(sceneIndex).arg(scene.scene_name);

        // 打印数据（十六进制格式）
        const QByteArray &bytes = scene.scene_data;
        QString hexLine;
        for (int i = 0; i < bytes.size(); ++i) {
            uint8_t b = static_cast<uint8_t>(bytes[i]);
            hexLine += QString("%1 ").arg(b, 2, 16, QLatin1Char('0')).toUpper();
        }
        qDebug() << QString("数据: %1").arg(hexLine);
    }
}
...
config_data_t AppProtocolSetConfig::getSceneConfigDataById(uint8_t scene_id) const
{
    ...
    // 1. 捞出对应的场景原始字节数据 (通常只有1条，因为 ID 唯一)
...
void AppProtocolSetConfig::deleteScene(uint8_t scene_id)
{
    ...
    m_panel_save.erase(std::remove_if(m_panel_save.begin(), m_panel_save.end(), [scene_id](const panel_save_t &p){
        return p.scene_id == scene_id;}), m_panel_save.end());
