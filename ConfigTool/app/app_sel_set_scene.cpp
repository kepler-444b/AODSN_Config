...
void DialogSetScene::populatePanels()
{
    ...
    for (const PanelInfo_t& panel : m_panels)
    {
...
void DialogSetScene::populateLeds()
{
    ...
    for (const ExtendInfo_t& ex : m_extends)
    {
        if (ex.type != LED_EX)
            continue;
...
void DialogSetScene::parseGroupBoxInfo(QGroupBox* group, uint8_t &addr, uint8_t &key_num, uint8_t &type)
{
    ...
    QRegExp rx("面板地址:(\\d+)\\s+按键数量:(\\d+)\\s+(.*)");
    if (rx.indexIn(title) != -1)
    {
        addr = rx.cap(1).toInt();
        key_num = rx.cap(2).toInt();
        QString typeStr = rx.cap(3).trimmed();
